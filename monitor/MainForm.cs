    ...
    public class MainForm : Form
    {
        ...
        private void LoadBannedIPs()
        {
            try
            {
                ...
                {
                    AppendLog($"[WARN] Can't read firewall rule RDP_BLOCK_ALL directly; showing merged data from block_list.log. Details: {firewallReadError}");
                }
        ...
        private IEnumerable<string> ReadBlockedTargetsFromFirewallRule()
        {
            ...
            {
                AppendLog($"[WARN] Failed to read firewall rule RDP_BLOCK_ALL via PowerShell: {ex.Message}");
            }
        ...
        private bool TryReadBlockedTargetsViaNetsh(HashSet<string> targets, out string errorDetails)
        {
            ...
            try
            {
                ...
                using (var process = Process.Start(psi))
                {
                    if (process == null)
                    {
                        errorDetails = "Failed to start netsh";
                        return false;
                    }
        ...
        private void RemoveBlockedTargetFromFirewallRule(string target)
        {
            using (var ps = PowerShell.Create())
            {
                ps.AddScript($@"
                    ...
                    $current = @((Get-NetFirewallRule -Name 'RDP_BLOCK_ALL' | Get-NetFirewallAddressFilter).RemoteAddress -split ',') |
        ...
        private bool EnsureConfigFileExists(string configPath)
        {
            try
            {
                ...
                File.WriteAllText(configPath, JsonSerializer.Serialize(defaultConfig, createOptions), Encoding.UTF8);
